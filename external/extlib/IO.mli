...
(** [input i b p l] reads up to [l] bytes from the given input, storing
  them in bytes [b], starting at byte number [p]. It returns the actual number
  of bytes read or raise [No_more_input] if no byte can be read. It will raise
  [Invalid_argument] if [p] and [l] do not designate a valid substring of [b].
  *)

val really_input : input -> bytes -> int -> int -> int
(** [really_input i b p l] reads exactly [l] bytes from the given input,
  storing them in the bytes [b], starting at position [p]. For consistency with
  {!IO.input} it returns [l]. Raises [No_more_input] if at [l] bytes are not
  available. Raises [Invalid_argument] if [p] and [l] do not designate a valid
  substring of [b]. *)
