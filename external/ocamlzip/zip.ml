...
let read_entry ifile e =
  try
    ...
    | Deflated ->
        ...
        let crc = Zlib.update_crc Int32.zero (String.of_bytes res) 0 (Bytes.length res) in
        if crc <> e.crc then
    ...
    | Deflated ->
        ...
        begin try
          Zlib.uncompress ~header:false
            ...
            (fun buf len ->
              output oc buf 0 len;
              crc := Zlib.update_crc !crc (String.of_bytes buf) 0 len)
        with Zlib.Error(_, _) ->
          raise (Error(ifile.if_filename, e.filename, "decompression error"))
        end;
        if !crc <> e.crc then
...
let write_directory_entry oc e =
  ...
  writestring oc (Bytes.of_string e.filename);            (* filename *)
  writestring oc (Bytes.of_string e.extra);               (* extra info *)
  writestring oc (Bytes.of_string e.comment)              (* file comment *)
...
let close_out ofile =
  ...
  writestring oc (Bytes.of_string ofile.of_comment);         (* comment *)
...
let add_entry_header ofile extra comment level mtime filename =
  ...
  writestring oc (Bytes.of_string filename);              (* filename *)
  writestring oc (Bytes.of_string extra);                 (* extra info *)
  { filename = filename;
    extra = extra;
    comment = comment;
      ...
      0 ->
        ...
        let rec copy sz =
          ...
          if r = 0 then sz else begin
            crc := Zlib.update_crc !crc (String.of_bytes buf) 0 r;
  ...
  | _ ->
      ...
      (fun buf pos len ->
        ...
        try
          send buf pos len;
          uncompr_size := !uncompr_size + len;
          crc := Zlib.update_crc !crc (String.of_bytes buf) pos len
        with Zlib.Error(_, _) ->
